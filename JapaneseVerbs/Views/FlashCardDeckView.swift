                    ...
                    VStack(alignment: .leading, spacing: 20) {
                        GroupBox(label: Text("Setup Flash Cards").font(.headline)) {
                            VStack(alignment: .leading, spacing: 16) {
                                Stepper("Number of Verbs: \(numberOfVerbs)", value: $numberOfVerbs, in: 5...50, step: 5)
                        ...
                        if !dataManager.selectedVerbs.isEmpty {
                            GroupBox(label: Text("Selected Verbs (\(dataManager.selectedVerbs.count))").font(.headline)) {
                        ...
                        GroupBox(label: Text("Quick Study").font(.headline)) {
                            Button(action: {
                                showingFlashCards = true
                                selectedCards = Array(dataManager.verbs.shuffled().prefix(numberOfVerbs))
                            }) {
                                Label("Start with \(numberOfVerbs) Random Verbs", systemImage: "shuffle")
                                    .frame(maxWidth: .infinity)
                                    .padding()
                        ...
                        GroupBox(label: Text("Tip").font(.headline)) {
                            Text("You can select verbs for study from the Browse tab. Swipe left on a verb or tap the star in its detail view to add it to your study deck.")
                                .font(.subheadline)
