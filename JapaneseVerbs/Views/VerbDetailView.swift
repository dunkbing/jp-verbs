...
struct VerbDetailView: View {
    ...
    private var basicInfoSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ConjugationCard(title: "Dictionary Form", content: [
                ConjugationRow(label: "Romaji", values: [verb.romaji]),
                ConjugationRow(label: "Japanese", values: !verb.presentIndicativePlainPositive.isEmpty ? [verb.presentIndicativePlainPositive[1]] : [])
            ])
            
            ConjugationCard(title: "Stem & Infinitive", content: [
            ...
            ConjugationCard(title: "Meaning", content: [
                ConjugationRow(label: "Positive", values: [verb.presentIndicativeMeaningPositive]),
                ConjugationRow(label: "Negative", values: [verb.presentIndicativeMeaningNegative])
            ])
        }
    }
    ...
    private var presentIndicativeSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ConjugationCard(title: "Plain Form", content: [
            ...
            ConjugationCard(title: "Polite Form", content: [
    ...
    private var progressiveSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ConjugationCard(title: "Present Progressive", content: [
                ConjugationRow(label: "Plain Positive", values: verb.presentProgressivePlainPositive),
                ConjugationRow(label: "Polite Positive", values: verb.presentProgressivePolitePositive),
                ConjugationRow(label: "Polite Negative", values: verb.presentProgressivePoliteNegative)
            ])
        }
    }
